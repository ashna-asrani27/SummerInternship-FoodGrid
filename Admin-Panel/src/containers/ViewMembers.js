...
const fieldss = ['name','email','membershipname','price','status', 'delete']
export default class ViewMembers extends Component {
  ...
  componentDidMount()
  {
    axios.get('http://localhost:8000/memberuser/')
    .then(response => {
  ...
  deleteCustomerRequest(id) {
    ...
    axios.delete('http://localhost:8000/memberuser/'+id)
  ...
  StatusRequest(id,status) {
        ...
        axios.post('http://localhost:8000/memberuser/'+id+'/'+status)
