...
//        router.route('/add').post(upload.array('restaurantimages', 8),async (req,res,next)=>{
//         try{
//             const reqFiles=[];
//             console.log("JHellpo")
...
//             // for(var i=0;i<req.files.length;i++)
//             // {
//             //     menuFiles.push(url + './public/' + req.menuimages[i].filename)
...
//                 return res
//                 .status(400)
//                 .json({ msg: "The Phonenumber needs to be at least 10 digit." });
...
router.route('/').get((req,res)=>{
    RestaurantRequest.find().limit(8)
    .then(restaurants=>res.json(restaurants))
    .catch(err=>res.status(400).json('Error: '+err));
});
